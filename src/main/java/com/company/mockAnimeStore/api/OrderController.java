...
public class OrderController {
    ...
    public Mono<EnrichedOrder> saveOrder(@RequestBody final Order order) {
        return service.saveOrder(order);
    };

    @PostMapping(value="orders",produces={APPLICATION_JSON_VALUE})
    @ResponseStatus(HttpStatus.CREATED)
    Flux<EnrichedOrder> saveManyOrder(@RequestBody final OrdersList ordersList) {
        return service.saveManyOrders(ordersList);
    };
}
